...
def main() -> None:
    ...
    with tabs[5]:
        st.subheader("Predictive Baseline: Next-Day Profitability")
        if daily.empty:
            st.info("`daily_trade_metrics.csv` not found.")
        else:
            ...
            if len(model_df) < 50:
                ...
                base_acc = (y_pred_base == y_test).mean()

                st.write(f"Majority baseline accuracy: **{base_acc:.3f}**")
                ...
                try:
                    ...
                    st.write(f"Logistic baseline accuracy: **{acc:.3f}**")
                    ...
                    st.dataframe(coef_df.head(8), width="stretch")
                except Exception as e:
                    st.info(f"scikit-learn unavailable for logistic baseline: {e}")
